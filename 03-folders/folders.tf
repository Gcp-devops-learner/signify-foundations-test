...
locals {
  ...
  sub_folders4_var = compact(flatten([for k, i in var.folder_map : length(i) == 0 ? [] : [for ip1, op1 in i : length(op1) == 0 ? [] : [for ip2, op2 in op1 : length(op2) == 0 ? [] : [for ip4, op3 in op2 : join("=3>", [join("=2>", [join("=1>", [k, ip1]), ip2]), ip4])]]]]))
...
module "sub_folders4" {
  ...
  version  = "~> 3.0"
  for_each = toset(local.sub_folders3_var)
  parent   = module.sub_folders3[element(split("=3>", each.value), 0)].id
  names    = [element(split("=3>", each.value), 1)]
}
